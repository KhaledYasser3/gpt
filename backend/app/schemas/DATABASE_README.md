...
2. **`subscriptions`**: فترات وصلاحيات الاشتراكات الفعالة للعملاء، يتم إنشاؤها وتحديد مدتها والـ Tokens بواسطة الإدارة مباشرة، وتعديلها أو تجديدها يدوياً.
3. **`chats` & `messages`**: حفظ تاريخ محادثات شات الذكاء الاصطناعي للمستخدمين.
4. **`usage_logs`**: تسجيل استهلاك الرموز (Tokens) ومعدل الاستخدام الفعلي.
5. **`settings`**: التحكم في متغيرات النظام العامة.
